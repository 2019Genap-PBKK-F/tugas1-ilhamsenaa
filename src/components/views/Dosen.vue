...
export default {
  ...
  methods: {
    newRow() {
      axios.post('http://10.199.14.46:8003/api/Aspek/', this.form).then(res => {
        console.log(res.data)
      })
    },
    updateRow(instance, cell, columns, row, value) {
      axios.get('http://10.199.14.46:8003/api/Aspek/').then(res => {
        var index = Object.values(res.data[row])
        index[columns] = value
        console.log(index)
        axios.put('http://10.199.14.46:8003/api/Aspek/' + index[0], {
          id: index[0],
          aspek: index[1],
    ...
    deleteRow(instance, row) {
      axios.get('http://10.199.14.46:8003/api/Aspek/').then(res => {
        ...
        // console.log(index)
        console.log(row)
        axios.delete('http://10.199.14.46:8003/api/Aspek/' + index[0])
      })
    }
  },
  computed: {
    jexcelOptions() {
      return {
        data: this.Aspek,
        allowToolbar: true,
        url: 'http://10.199.14.46:8003/api/Aspek/',
