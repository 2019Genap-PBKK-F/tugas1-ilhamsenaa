...
export default {
  ...
  methods: {
    newRow() {
      axios.post('http://10.199.14.41:8003/api/Capaian_Unit/', this.form).then(res => {
        console.log(res.data)
      })
    },
    updateRow(instance, cell, columns, row, value) {
      axios.get('http://10.199.14.41:8003/api/Capaian_Unit/').then(res => {
        var index = Object.values(res.data[row])
        index[columns] = value
        console.log(index)
        axios.put('http://10.199.14.41:8003/api/Capaian_Unit/' + index[0], {
    ...
    deleteRow(instance, row) {
      axios.get('http://10.199.14.41:8003/api/Capaian_Unit/').then(res => {
        ...
        // console.log(index)
        console.log(row)
        axios.delete('http://10.199.14.41:8003/api/Capaian_Unit/' + index[0])
      })
    }
  },
  computed: {
    jexcelOptions() {
      return {
        ...
        allowToolbar: true,
        url: 'http://10.199.14.41:8003/api/Capaian_Unit/',
